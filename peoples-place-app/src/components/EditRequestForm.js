...
function EditRequestForm({ productToEdit, onUpdateProduct }) {
  ...
  useEffect(() => {
    fetch(`http://localhost:4000/products/${productToEdit.id}`)
      .then((res) => res.json())
  ...
  const handleSubmit = (e) => {
    ...
    fetch(`http://localhost:4000/products/${productToEdit.id}`, configObj)
      .then((resp) => resp.json())
      .then((updatedProd) => {
